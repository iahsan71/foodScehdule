...
function LandingPage() {
    ...
    let stateData = useRef();
    const uid = useSelector((state) => state.authUser.uid);
    const { list, lastDocument, hasMoreDocument } = useSelector(
        (state) => state.food
    );
    ...
    const handleAdd = () => {
        if (foodName && day) {
            ...
            if (!editing.id) {
                ...
                dispatch(
                    addFoodSchedule(newSchedule, () => {
                        setLoadingAdd(false);
                        setFoodName("");
                        setDay("");
                        setSelectedImage(null);
                        stateData.current.value = "";
                    })
                );
            } else {
                ...
                dispatch(
                    editFoodSchedule(editing.id, updatedSchedule, () => {
                        setLoadingAdd(false);
                        setEditing({ id: null });
                        setFoodName("");
                        setDay("");
    ...
    const handleDelete = (id, imageUrl) => {
        setLoadingDelete(id);
        dispatch(
            deleteFoodSchedule(id, imageUrl, () => {
    ...
    const filteredFoodSchedule = filterDay
        ? list.filter((item) => item.day === filterDay)
        : list;
